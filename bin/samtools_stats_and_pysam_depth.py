...
# update script to take in pysamstats parsed file instead
def depth_stats(depth_file, sample_name):
    # edit to change to pysamstats parsed file, needs to get cut by gagpol coordinates first
    data=pd.read_csv(depth_file, usecols=["pos", "reads_all"], index_col="pos")
    print(data)
    ...
    # Median
    median_depth = data['reads_all'].median()
    print("This is the median depth: ", median_depth)

    # Mean absolute deviation
    mad_depth = (data['reads_all'] - data['reads_all'].median()).abs().median()
