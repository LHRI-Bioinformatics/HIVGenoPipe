process REPORT_NGS_STATS{
    ...
    input:
    tuple val(meta), path(trim_stats), path(sealstats), path(self_align_bam), path(pysamstats_fastas), path(samtools_depth_and_stats), path(contig_coverage)
    ...
    -s ${self_align_bam} -l 100 -p ${pysamstats_fastas} \\
    -d ${samtools_depth_and_stats} -c ${contig_coverage}
