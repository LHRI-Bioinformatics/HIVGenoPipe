...
//file("/mnt/LHRI/Members/ldotrang/HIVGenoPipe/HIVGenoPipe_Dev_controls_testing/assets/NL4_3.fasta", checkIfExists: true)
...
include { REPORT_LOG_REPORT                           } from '../modules/local/report/log_report/main'
...
include { REPORT_STANFORD_SIERRA                      } from '../modules/local/report/stanford_sierra/main'
include { REPORT_STANFORD_SUMMARY                     } from '../modules/local/report/stanford_summary/main'
// add realignment of final consensus for final stats
include { BWA_INDEX as BWA_INDEX_3                    } from '../modules/nf-core/modules/bwa/index/main_edit'
include { BWA_MEM as BWA_MEM_3                        } from '../modules/nf-core/modules/bwa/mem/main_edit'
include { SAMTOOLS_INDEX as SAMTOOLS_INDEX_2          } from '../modules/nf-core/modules/samtools/index/main'
include { SAMTOOLS_STATS                              } from '../modules/nf-core/modules/samtools/stats/main_edit'
include { PYSAMSTATS as PYSAMSTATS_2                  } from '../modules/local/pysamstats/pysamstats/main'
include { PYSAMSTATS_PARSER as PYSAMSTATS_PARSER_2    } from '../modules/local/pysamstats/parser/main'
include { REPORT_GAGPOL_QUALITY                       } from '../modules/local/report/gagpol_quality/main'
...
// 032026 update - need to add final alignment step to get real stats for the end, get alignment for gagpol like production is doing
...
// input stuff
    ...
    ch_ngs_stats_all_report =  REPORT_NGS_STATS_ALL.out.ngs_stats.collect()
  ...
  //Map the bai to the bam for use in pysamstats
    ...
    graphql_query = file("${projectDir}/bin/sierra.gql")

    REPORT_STANFORD_SIERRA (
        PYSAMSTATS_PARSER.out.fastas,
        graphql_query
    )

    ch_json_combine = Channel.empty()
    ch_json_combine = REPORT_STANFORD_SIERRA.out.individual_json.collect{it[1]}
...
// final alignment and stats gathering
    BWA_INDEX_3 (
        PYSAMSTATS_PARSER.out.consensus_fasta
    )
    ch_bwa_mem_input_3 = TRIMMOMATIC.out.trimmed_reads.join(BWA_INDEX_3.out.index)
  ...
  //Map the bai to the bam for use in pysamstats
    BWA_MEM_3.out.bam
        .join(SAMTOOLS_INDEX_2.out.bai, by: [0], remainder: true)
        .map {
            meta, bam, bai -> [ meta, bam, bai ]
        }
        .set { ch_bam_bai_final }
    ch_pysamstats_input_final = ch_bam_bai_final.join(PYSAMSTATS_PARSER.out.consensus_fasta)
    ...
    PYSAMSTATS_2 (
        ch_pysamstats_input_final
    )

    PYSAMSTATS_PARSER_2 (
        PYSAMSTATS_2.out.pysamstats_tsv
    )


    ch_gagpol_quality = REPORT_STANFORD_SIERRA.out.amb_jsons.join(PYSAMSTATS_PARSER_2.out.parsed_tsv)
    ch_gagpol_quality = ch_gagpol_quality.join(PYSAMSTATS_PARSER_2.out.consensus_fasta)

    REPORT_GAGPOL_QUALITY(
        ch_gagpol_quality
    )

    ch_depth_and_stats = Channel.empty()
    // may need to edit this once we change gagpol output
    ch_depth_and_stats = SAMTOOLS_STATS.out.stats.join(REPORT_GAGPOL_QUALITY.out.truncated_pysamstats) 
    ...
    ch_ngs_stats = ch_ngs_stats.join(BWA_MEM_3.out.bam)
    ...
    ch_execution_log = file("$params.outdir/pipeline_info/execution_trace*.txt", checkIfExists: true)
    ...
    db_jsons = REPORT_STANFORD_SIERRA.out.individual_json.collect{it[1]}
